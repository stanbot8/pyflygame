...
class TestGaitPhaseWrap:
    def test_phase_stays_in_range_forward(self):
        g = GaitState()
        cmd = MotorCommand(forward_velocity=30.0)
        for _ in range(10000):
            g.update(cmd, 0.01)
        assert 0.0 <= g.phase < 2.0 * math.pi

    def test_phase_stays_in_range_reverse(self):
        g = GaitState()
        cmd = MotorCommand(forward_velocity=-20.0)
        for _ in range(10000):
            g.update(cmd, 0.01)
...
class TestBodyState:
    def test_roundtrip(self):
        ...
        orig = BodyState(
            joint_angles=[float(i) for i in range(42)],
            joint_velocities=[float(i) * 0.1 for i in range(42)],
            contacts=[1.0, 0.0, 1.0, 0.0, 1.0, 0.0],
            body_velocity=[10.0, 0.5, -0.1],
        ...
        packed = orig.pack()
        restored = BodyState.unpack(packed)
        for i in range(42):
            assert abs(restored.joint_angles[i] - orig.joint_angles[i]) < 1e-5
            assert abs(restored.joint_velocities[i] - orig.joint_velocities[i]) < 1e-5
        for i in range(6):
            assert abs(restored.contacts[i] - orig.contacts[i]) < 1e-5
        for i in range(3):
            assert abs(restored.body_velocity[i] - orig.body_velocity[i]) < 1e-5
            assert abs(restored.position[i] - orig.position[i]) < 1e-5
        assert abs(restored.heading - orig.heading) < 1e-5
        assert abs(restored.sim_time - orig.sim_time) < 1e-3
        assert restored.step == orig.step

    def test_default_size(self):
        from nmfly.adapters.fwmc import BodyState, BODY_STATE_SIZE
        bs = BodyState()
        packed = bs.pack()
        assert len(packed) == BODY_STATE_SIZE


class TestStimCommand:
    def test_unpack(self):
        import struct
        from nmfly.adapters.fwmc import StimCommand, STIM_COMMAND_FMT
        # Pack manually: neuron_idx=5, intensity=0.8, excitatory=1, duration=2.0
        raw = struct.pack(STIM_COMMAND_FMT, 5, 0.8, 1, 2.0)
        cmd = StimCommand.unpack(raw)
        assert cmd.neuron_idx == 5
        assert abs(cmd.intensity - 0.8) < 1e-5
        assert cmd.excitatory is True
        assert abs(cmd.duration_ms - 2.0) < 1e-5
